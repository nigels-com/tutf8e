...
with open('src/tutf8e.c', 'w') as src:
  ...
  src.write('\n')
  src.write('int encode_utf8(const uint16_t *table, const char **i, size_t *ileft, char **o, size_t *oleft)\n')
  src.write('{\n')
  src.write('  for (;;*i += 1, *ileft -= 1) {\n')
  src.write('    uint16_t c = table[*i[0]];\n')
  src.write('    if (c<0x80) {\n')
...
with open('include/tutf8e.h', 'w') as include:
  ...
  for e in sorted(encodings):
    ...
    with open('src/%s.c'%(name), 'w') as src:
      ...
      src.write('{\n')
      src.write('  size_t len = strlen(src);\n')
