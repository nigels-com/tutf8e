...
char * encode_windows_1252_to_utf8(const char *input)
{
  size_t ilen = strlen(input) + 1;
  size_t olen = 0;
  if (!length_utf8(windows_1252_utf8, input, ilen, &olen) && olen) {
    char * output = malloc(olen);
    if (!encode_utf8(windows_1252_utf8, input, ilen, output, olen)) {
      return output;
    }
    free(output);
  }
