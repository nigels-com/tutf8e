...
char * encode_iso_8859_3_to_utf8(const char *input)
{
  size_t ilen = strlen(input) + 1;
  size_t olen = 0;
  if (!length_utf8(iso_8859_3_utf8, input, ilen, &olen) && olen) {
    char * output = malloc(olen);
    if (!encode_utf8(iso_8859_3_utf8, input, ilen, output, olen)) {
      return output;
    }
    free(output);
  }
...
int encode_iso_8859_3_utf8(char *dest, size_t size, const char *src)
{
  ...
  return encode_utf8(iso_8859_3_utf8, src, len, dest, size);
}
