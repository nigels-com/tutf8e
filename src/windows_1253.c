...
int tutf8e_string_encode_windows_1253(char *output, size_t olen, const char *input)
{
  size_t len = strlen(input) + 1;
  return tutf8e_buffer_encode(tutf8e_windows_1253_utf8, input, len, output, &olen);
}

int tutf8e_buffer_length_windows_1253(const char *i, size_t ilen, size_t *length)
{
  return tutf8e_buffer_length(tutf8e_windows_1253_utf8, i, ilen, length);
}

int tutf8e_buffer_encode_windows_1253(char *output, size_t *olen, const char *input, size_t ilen)
{
  return tutf8e_buffer_encode(tutf8e_windows_1253_utf8, input, ilen, output, olen);
}

char * tutf8e_string_encode_windows_1253_realloc(char *input)
{
  size_t ilen = 0;
  size_t olen = 0;
  if (input && !tutf8e_string_length(tutf8e_windows_1253_utf8, input, &ilen, &olen) && ilen && olen && ilen!=olen) {
    char * output = malloc(olen + 1);
    if (output && !tutf8e_buffer_encode(tutf8e_windows_1253_utf8, input, ilen, output, &olen)) {
      free(input);
      output[olen] = 0;
      return output;
    }
